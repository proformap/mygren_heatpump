...
class MygrenSwitch(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        if self.entity_description.key == "tuv_scheduler_enabled":
            await self.hass.async_add_executor_job(
                self._api.set_tuv_scheduler_enabled, True
            )
        elif self.entity_description.key == "program_scheduler_enabled":
            await self.hass.async_add_executor_job(
                self._api.set_program_scheduler_enabled, True
            )
        elif self.entity_description.key == "tariff_watch":
            await self.hass.async_add_executor_job(
                self._api.set_tariff_watch, True
            )
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        ...
        if self.entity_description.key == "tuv_scheduler_enabled":
            await self.hass.async_add_executor_job(
                self._api.set_tuv_scheduler_enabled, False
            )
        elif self.entity_description.key == "program_scheduler_enabled":
            await self.hass.async_add_executor_job(
                self._api.set_program_scheduler_enabled, False
            )
        elif self.entity_description.key == "tariff_watch":
            await self.hass.async_add_executor_job(
                self._api.set_tariff_watch, False
            )
