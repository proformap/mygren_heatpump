...
class MygrenClimate(CoordinatorEntity, ClimateEntity):
    ...
    @staticmethod
    def _mode_from_data(data: dict) -> HVACMode:
        """Derive HVAC mode from raw coordinator data (no optimistic check)."""
        if not data.get("hp_enabled"):
            return HVACMode.OFF
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        This prevents the UI from reverting to old values when the
        coordinator polls faster than MaR processes the PUT command.
        """
        data = self.coordinator.data

        if self._optimistic_target_temp is not None:
            actual = data.get("comfort")
            if actual is not None and int(actual) == int(self._optimistic_target_temp):
                self._optimistic_target_temp = None

        if self._optimistic_hvac_mode is not None:
            if self._mode_from_data(data) == self._optimistic_hvac_mode:
